...
const StepthreeView = () => {
  ...
  useEffect(() => {
    ...
    fetchBankData(user_id);
    fetchVahnData(user_id);
    fetchBillData(user_id);
  ...
  const fetchBankData = async (user_id) => {
    try {
  ...
  const fetchVahnData = async (user_id) => {
    try {
      const response = await axios.get(
  ...
  const handleVahnUpdateAll = async () => {
    ...
    if (!user_id || !Array.isArray(vehicles) || vehicles.length === 0) {
      alert("उपयोगकर्ता ID या वाहन डेटा उपलब्ध नहीं है।");
      return;
    }
    ...
    try {
        ...
        .map((item) => {
          ...
          return axios.put(
            `https://brjobsedu.com/Nandagora/api2/Nandavahanupdateview/${user_id}/`,
            {
              ...
              district: "dehradun",
              project: "chakarata",
              vec_model: item.vec_model,
              vec_number: item.vec_number,
              vec_amt: item.vec_amt,
              vec_other: item.vec_other,
            }
          );
        })
        .filter(Boolean);
      ...
      console.log("data return", updateRequests);
      alert("सभी वाहन विवरण सफलतापूर्वक अपडेट हो गए।");
    } catch (err) {
      if (err.response) {
        console.error("वाहन विवरण अपडेट त्रुटि:", err.response.data);
      } else if (err.request) {
        console.error("सर्वर से कोई उत्तर नहीं मिला:", err.request);
      } else {
        console.error("अनपेक्षित त्रुटि:", err.message);
  ...
  const fetchBillData = async (user_id) => {
    try {
      const response = await axios.get(
  ...
  const handleUpdateBill = async () => {
    ...
    if (!user_id || !Array.isArray(bills) || bills.length === 0) {
      alert("उपयोगकर्ता ID या बिल डेटा उपलब्ध नहीं है।");
      return;
    }
  ...
  return (
    <div className="container my-4 nd-step1">
...
<div className="nd-step1">
      ...
      {bankData.length > 0 && (
        <div className="nd-update-detials mt-3">
          <Button className="nd-primary-dawnload" onClick={handleBankUpdateAll}>
            Update All Bank Records
          </Button>
        </div>
      )}

      <h5 className="mt-4">वाहन का विवरण:</h5>
      <Table bordered responsive>
        <thead className="nd-born-thead">
          <tr>
            <th>क्र.सं.</th>
      ...
      {vehicles.length > 0 && (
        <div className="nd-update-detials mt-3">
          <Button className="nd-primary-dawnload" onClick={handleVahnUpdateAll}>
            Update All Vehicle Records
          </Button>
        </div>
      )}

      <h5 className="mt-4">परिवार के विगत 03 बार के बिजली के बिलों का विवरण:</h5>
      <Table bordered responsive>
        <thead className="nd-born-thead">
          <tr>
            <th>क्र.सं.</th>
        ...
        <tbody>
          {bills.length === 0 ? (
            <tr>
              <td colSpan="5">कोई बिल डेटा उपलब्ध नहीं है</td>
            </tr>
          ) : (
