...
const OrphanFormStatus = () => {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([fetchStep1, fetchStep2])
      .then((results) => {
        const step1Data =
  ...
  const isStepComplete = (step) => {
    switch (step) {
      ...
      case 2:
        if (!formData.step2) return false;
        return requiredFilesStep2.every((fileKey) => !!formData.step2[fileKey]);
      case 3:
  ...
  const allStepsCompleted = [1, 2, 3].every((step) =>
    isStepComplete(step)
  );
  ...
  const handleProceedClick = () => {
    if (firstIncompleteStep)
      navigate(stepToRouteMap[firstIncompleteStep] || "/OrphanStepOne");
  };
  ...
  <>
    <Accordion defaultActiveKey="1">
      <Accordion.Item eventKey="0">
        <Accordion.Header>Orphan Girls</Accordion.Header>
        <Accordion.Body>
          <div className="mt-4">
...
export default OrphanFormStatus;
